...
No. It does not save command history by default. Browser storage is used for preferences such as display options and custom command prefixes. If you explicitly click the local stash action, only the repaired command is saved in the current browser with an expiration time.

### What is local stash?

Local stash is an optional temporary save area for repaired commands. It is not automatic history. It stores only repaired command text after you explicitly choose to stash it, keeps it in the current browser, and automatically removes expired items.
...
Yes, the MVP is designed around real failures such as Python `-c` commands with long SQL strings. It can repair high-confidence breaks in dates, SQL identifiers, field names, filenames, paths, and related tokens.
...
默认不会。浏览器本地存储只用于保存显示偏好和自定义命令前缀。只有当你主动点击“本地暂存”时，工具才会把修复后的命令保存在当前浏览器本地，并按过期时间自动清理。

### 什么是本地暂存？

本地暂存是一个可选的临时保存区，用来保存修复后的命令。它不是自动历史记录。只有你主动暂存时才会保存修复后的命令文本，内容只保存在当前浏览器本地，并会自动清理过期项。
...
支持。MVP 的核心场景之一就是 Python `-c` 中包含长 SQL 字符串的折行问题。它可以修复日期、SQL 标识符、字段名、文件名、路径等高置信断点。
