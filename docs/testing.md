...
- P1：本地暂存支持自动生成标题、编辑标题、读取旧版无标题数据、删除单条和清空全部，且不保存原始输入、修复点、风险详情、hash、路径或 URL 等敏感派生字段。
...
15. 点击单条结果旁的“暂存”，确认只保存修复后的完整命令并自动生成短标题；localStorage 暂存项只包含 `id`、`title`、`command`、`createdAt`、`expiresAt`。
16. 在本地暂存区编辑标题，确认输入后立即提示已保存，刷新后标题仍保留；手工造一条旧版无 `title` 暂存项，确认页面可读取并补默认标题。
...
24. 移动端窄屏确认页面变为单列，检查编辑框、复制/暂存/恢复按钮、标题输入框和修复点列表不互相遮挡，长命令可横向滚动或完整换行查看。
